...
public sealed class FileSystemObjectStorage(IOptions<FileSystemObjectStorageOptions> options) : IBlobStorage
{
    ...
    private readonly string _root = Path.GetFullPath(
        string.IsNullOrWhiteSpace(options.Value.RootPath)
            ? throw new InvalidOperationException($"{nameof(FileSystemObjectStorageOptions)}.{nameof(FileSystemObjectStorageOptions.RootPath)} must be set.")
    ...
    public async Task<StorageObject?> GetAsync(string key, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            return new StorageObject(BuildInfo(key, new FileInfo(contentPath), metadata), stream);
        }
    ...
    private static StorageObjectInfo BuildInfo(string key, FileInfo file, StoredMetadata? metadata) => new()
    {
        Key = key,
        Size = file.Length,
    ...
    private async Task WriteMetadataAsync(string key, PutObjectOptions? options, CancellationToken cancellationToken)
    {
        var metaPath = ResolveMetadataPath(key);
