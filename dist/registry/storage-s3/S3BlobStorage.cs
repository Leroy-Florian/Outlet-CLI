...
/// provider-specific <see cref="IS3BlobStorage"/> from one class — registered once and
/// forwarded to both interfaces (see AddS3BlobStorage). Thin by design: resilience is
/// composed over the port, never embedded here.
/// </summary>
public sealed class S3BlobStorage : IS3BlobStorage, IDisposable
{
    ...
    public S3BlobStorage(IOptions<S3BlobStorageOptions> options)
    {
        var value = options.Value;
        _bucket = string.IsNullOrWhiteSpace(value.BucketName)
            ? throw new InvalidOperationException($"{nameof(S3BlobStorageOptions)}.{nameof(S3BlobStorageOptions.BucketName)} must be set.")
            : value.BucketName;
    ...
    public async Task<Blob?> GetAsync(string key, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            return new Blob(BuildInfo(key, response), response.ResponseStream);
        }
    ...
    private static BlobInfo BuildInfo(string key, GetObjectResponse response)
    {
