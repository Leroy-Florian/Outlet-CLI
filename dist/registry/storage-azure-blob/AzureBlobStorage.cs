...
/// </summary>
public sealed class AzureBlobStorage : IAzureBlobStorage
{
    ...
    public AzureBlobStorage(IOptions<AzureBlobStorageOptions> options)
    {
        ...
        if (string.IsNullOrWhiteSpace(value.ConnectionString))
            throw new InvalidOperationException($"{nameof(AzureBlobStorageOptions)}.{nameof(AzureBlobStorageOptions.ConnectionString)} must be set.");
        if (string.IsNullOrWhiteSpace(value.ContainerName))
            throw new InvalidOperationException($"{nameof(AzureBlobStorageOptions)}.{nameof(AzureBlobStorageOptions.ContainerName)} must be set.");
    ...
    public async Task<Blob?> GetAsync(string key, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            return new Blob(BuildInfo(key, result.Details), result.Content);
        }
    ...
    private static BlobInfo BuildInfo(string key, BlobDownloadDetails details) => new()
    {
        Key = key,
