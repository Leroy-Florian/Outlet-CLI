...
public sealed class InMemoryObjectStorage : IBlobStorage
{
    ...
    public InMemoryObjectStorage(IOptions<InMemoryObjectStorageOptions> options)
    {
    ...
    public Task PutAsync(string key, Stream content, PutObjectOptions? options = null, CancellationToken cancellationToken = default)
    {
    ...
    public Task<StorageObject?> GetAsync(string key, CancellationToken cancellationToken = default)
    {
        ...
        return Task.FromResult<StorageObject?>(new StorageObject(entry.ToInfo(key), stream));
    }
    ...
    {
        public StorageObjectInfo ToInfo(string key) => new()
        {
            Key = key,
            Size = Content.Length,
